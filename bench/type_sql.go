...
const CreateUserActiveStmt = `
CREATE INDEX IF NOT EXISTS user_active ON users (user_active)
`

const SelectUserActiveStmt = `
SELECT 
 user_id
,user_name
,user_pass
...
const SelectUserActiveRangeStmt = `
SELECT 
 user_id
,user_name
,user_pass
...
const SelectUserActiveCountStmt = `
SELECT count(1)
FROM users 
WHERE user_active=?
`
